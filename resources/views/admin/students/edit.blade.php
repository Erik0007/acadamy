                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" rows="2"
                                class="w-full border-gray-300 rounded shadow-sm focus:ring-indigo-500">{{ old('address', $student->address) }}</textarea>
                        </div>
                                ...
                                @foreach($courses as $course)
                                    <label class="flex items-center gap-2 p-2 border rounded cursor-pointer hover:bg-indigo-50">
                                        <input type="checkbox" name="courses[]" value="{{ $course->id }}"
                                            {{ in_array($course->id, old('courses', $assignedCourses)) ? 'checked' : '' }}
